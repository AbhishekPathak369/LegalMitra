...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [statsResponse, recentUsersResponse, allUsersResponse, verificationsResponse, analyticsResponse] = await Promise.all([
                ...
                fetch(`${API_BASE_URL}/admin/users/recent`, { headers: getHeaders() }),
                fetch(`${API_BASE_URL}/admin/users`, { headers: getHeaders() }),
                fetch(`${API_BASE_URL}/admin/verifications/pending`, { headers: getHeaders() }),
                fetch(`${API_BASE_URL}/admin/analytics`, { headers: getHeaders() })
            ]);
  ...
  try {
    ...
    console.log('📤 Sending reject request to:', `${API_BASE_URL}/admin/verifications/${id}/reject`);
...
const DashboardOverview = ({ statsData, recentUsers }) => { 
    ...
    useEffect(() => {
        const fetchChartData = async () => {
            try {
                const token = localStorage.getItem('token');
                const response = await fetch(`${API_BASE_URL}/admin/analytics/charts?range=${timeRange}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    }
...
const ExportModal = ({ onClose, onExportData }) => {
    ...
    const fetchExportStatistics = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:5000/api/admin/export/statistics', {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
...
const AnalyticsSettings = ({ analyticsData, statsData }) => {
    ...
    const toggleSetting = async (settingName) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_BASE_URL}/admin/settings/${settingName}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
